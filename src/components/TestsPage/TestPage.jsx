...
const TestPage = () => {
    ...
    useEffect(() => {
        setTimeout(()=>{dispatch(getTestData(num));
            dispatch(setSaveResault());
            window.scrollTo(0, 0)
            setActive(true)},300)//таймаут для анімації щоб не появлявся новий тест до того як пройде анімація
    }, [num]);
    return <div>{num === 7 ? <FinishTests /> : <Test active={active} setActive={setActive}/>}</div>;
};
