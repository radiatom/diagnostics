...
export const setResaultDiagnosticData = (
    resault: resaultType
): setResaultDiagnosticDataType => {
    return {
...
const testReducer = (
    state: standartStateTestType = standartStateTest,
    action: actionsTypes
): standartStateTestType => {
    switch (action.type) {
        case SET_TEST_DATA:
            return {
                ...state,
...
export const getTestData =
    (linkNumber: number) => (dispatch: Dispatch<actionsTypes>) => {
        let testData = {};
...
export const getResaultDiagnosticData =
...
export const setSaveResault = () => (dispatch: Dispatch<actionsTypes>) => {
    const res = JSON.parse(
        sessionStorage.getItem("res") || '{"error":"no res"}'
    );
...
export const putSolutionTestTrue =
    (linkSolution: number) => (dispatch: Dispatch<actionsTypes>) => {
        serverData.resaultDiagnostic.forEach((el) => {
...
export const putSolutionTestFalse =
    (linkSolution: number) => (dispatch: Dispatch<actionsTypes>) => {
        serverData.resaultDiagnostic.forEach((el) => {
            if (el.id === linkSolution) {
                return (el.solution = false);
